...
public class servlet1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        counterRequest = (counterClass) request.getAttribute("counterRequest");
        if (counterRequest == null) {
            counterRequest = new counterClass();
            request.getSession().setAttribute("counterRequest", counterRequest);
        }

        counterSession = (counterClass) request.getSession().getAttribute("counterSession");
        if (counterSession == null) {
            counterSession = new counterClass();
            request.getSession().setAttribute("counterSession", counterSession);
        }

        counterApplication = (counterClass) request.getServletContext().getAttribute("counterApplication");
        if (counterApplication == null) {
            counterApplication = new counterClass();
            request.getSession().setAttribute("counterApplication", counterApplication);
        }
